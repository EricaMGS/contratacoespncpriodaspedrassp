...
def identificador_compra(row: Any) -> Tuple[Optional[str], Optional[int], Optional[int]]:
    r_dict = row.to_dict() if hasattr(row, 'to_dict') else dict(row)
    cnpj = cnpj_limpo(primeiro(r_dict, ["cnpjOrgao", "cnpj", "cnpjCompra"], CNPJ))
    ano = primeiro(r_dict, ["anoCompra", "ano", "anoContratacao"], None)
    seq = primeiro(r_dict, ["sequencialCompra", "sequencialContratacao", "sequencial"], None)
    ...
    if ano is None or seq is None:
        a, s = extrair_ano_seq_controle(primeiro(r_dict, ["numeroControlePNCPCompra", "numeroControlePNCP"], ""))
...
def identificador_contrato(row: Any) -> Tuple[Optional[str], Optional[int], Optional[int]]:
    r_dict = row.to_dict() if hasattr(row, 'to_dict') else dict(row)
    cnpj = cnpj_limpo(primeiro(r_dict, ["cnpjOrgao", "cnpj"], CNPJ))
    ano = primeiro(r_dict, ["anoContrato", "anoContratoEmpenho", "ano"], None)
    seq = primeiro(r_dict, ["sequencialContrato", "sequencialContratoEmpenho", "sequencial"], None)
    ...
    if ano is None or seq is None:
        a, s = extrair_ano_seq_controle(primeiro(r_dict, ["idContratoPNCP", "numeroControlePNCP"], ""))
        ano, seq = ano or a, seq or s
